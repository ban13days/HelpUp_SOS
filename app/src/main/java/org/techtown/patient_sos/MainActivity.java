...
public class MainActivity extends AppCompatActivity implements SensorEventListener{
    ...
    private boolean isBixbyEnabled = false;
    private  boolean  isVoiceRequestEnabled= false;
    private  boolean isHelpRequestEnabled = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 스위치 상태 가져오기
        isShakeEnabled = sharedPreferences.getBoolean("ShakeSOS", false);
        isBixbyEnabled = sharedPreferences.getBoolean("BixbySOS", false);
        isVoiceRequestEnabled = sharedPreferences.getBoolean("VoiceRequest", false);
        isHelpRequestEnabled = sharedPreferences.getBoolean("HelpRequest", false);
        ...
        textToSpeech = new TextToSpeech(this, status -> {
            if (status == TextToSpeech.SUCCESS) {
                ...
                if (isVoiceRequestEnabled) {
                    if(isShakeEnabled && !isBixbyEnabled) {
                        textToSpeech.speak("SOS실행을 원하실 경우 휴대폰을 흔들어주세요.", TextToSpeech.QUEUE_FLUSH, null, null);
                    }
                    else if(isBixbyEnabled)
                    {
                        textToSpeech.speak(time+"초 후에 SOS가 실행됩니다", TextToSpeech.QUEUE_FLUSH, null, null);
                    }
                }
            }
        });
        ...
        time = sharedPreferences.getInt("Timer", 10); // 기본값 10

        //상세위치 불러오기
        DetailLocation = sharedPreferences.getString("DetailLocation", ""); // 기본값은 빈 문자열
        textfield=findViewById(R.id.textfield);
        textfield.setText(DetailLocation);

        // 도움요청 메세지값 불러오기
        savedMessage = sharedPreferences.getString("helpmessage", " ");
        ...
        save.setOnClickListener(v -> {
            //DetailLocation = textField.getText() != null ? textField.getText().toString().trim() : "";
            DetailLocation = textField.getText().toString();
            ...
            if (DetailLocation.isEmpty()) {
                ...
                sharedPreferences.edit().putString("DetailLocation", DetailLocation).apply(); // SharedPreferences에 저장
        ...
        handleSelectedPatientType(isShakeEnabled,isBixbyEnabled,isVoiceRequestEnabled,isHelpRequestEnabled);
    }
    ...
    private void sendSMSWithLocation(String address) {
        ...
        if (DetailLocation != null && !DetailLocation.isEmpty()) {
            message += "\n상세위치: " + DetailLocation;
        }
        else {
    ...
    private void sendSMS(String phoneNumber, String message, String address) {
        try {
            ...
            if(isVoiceRequestEnabled)
            {
                textToSpeech.speak("SOS작동완료.. 현재 위치는"+address+"입니다.", TextToSpeech.QUEUE_FLUSH, null, null);
                Thread.sleep(7000);
                if(isHelpRequestEnabled)
                {
                    startTTSRepeatingMessage();
                }
            }

            else if(!isVoiceRequestEnabled)
            {
                if(isHelpRequestEnabled)
                {
                    startTTSRepeatingMessage();
            ...
            if(isVoiceRequestEnabled)
            {
                textToSpeech.speak("SOS작동실패.. 재시도 바람", TextToSpeech.QUEUE_FLUSH, null, null);
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SETTINGS_REQUEST_CODE && resultCode == RESULT_OK) {
            if (data != null) {
                ...
                // Switch 상태 가져오기
                boolean isShakeEnabled = sharedPreferences.getBoolean("ShakeSOS", false);
                boolean isBixbyEnabled = sharedPreferences.getBoolean("BixbySOS", false);
                boolean isVoiceRequestEnabled = sharedPreferences.getBoolean("VoiceRequest", false);
                boolean isHelpRequestEnabled = sharedPreferences.getBoolean("HelpRequest", false);

                // SeekBar 값 가져오기
                time = data.getIntExtra("Timer", sharedPreferences.getInt("Timer", 10)); // Intent 값 우선, 없으면 SharedPreferences
                sensitivity = sharedPreferences.getInt("Sensitivity", 10);

                // 변경된 설정 적용
                Log.d("DEBUG", "onActivityResult에서 업데이트된 time 값: " + time);
                Log.d("DEBUG", "onActivityResult에서 업데이트된 sensitivity 값: " + sensitivity);
                handleSelectedPatientType(isShakeEnabled, isBixbyEnabled, isVoiceRequestEnabled, isHelpRequestEnabled);
            }
        }
            ...
            if (data != null) {
                ...
                Log.d("DEBUG", "onActivityResult에서 받은 메시지: " + message);
            }
        }
    }

    //기능구현 ---------------------------------------------------------------------------------------
    private void handleSelectedPatientType(boolean isShakeEnabled,
                                           boolean isBixbyEnabled,
                                           boolean isVoiceRequestEnabled,
                                           boolean isHelpRequestEnabled) {
        if (isShakeEnabled) {
            Toast.makeText(this, "흔들기 모드", Toast.LENGTH_SHORT).show();
        }
        if (isBixbyEnabled) {
            Toast.makeText(this, "앱과 동시 실행 모드", Toast.LENGTH_SHORT).show();
            showSOSDialog();
        }
        if (isVoiceRequestEnabled) {
            if(isShakeEnabled && !isBixbyEnabled) {
...
public void onSensorChanged(SensorEvent event) {
        ...
        //심근경색 환자일때
        if (isShakeEnabled && event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
    ...
    private void showSOSDialog() {
        ...
        builder.setMessage(time+"초 후 SOS가 실행됩니다.")
                .setCancelable(false);
    ...
    private void startTTSRepeatingMessage() {
        ...
        if (isHelpRequestEnabled) {
            countDownTimer = new CountDownTimer(Long.MAX_VALUE, 4000) {
                ...
                public void onTick(long millisUntilFinished) {
                    if (textToSpeech != null) {
                        textToSpeech.speak(savedMessage, TextToSpeech.QUEUE_FLUSH, null, null);
                    }
                }

                @Override
                public void onFinish() {
                    // 타이머가 자동으로 끝날 일은 없으니 비워둡니다.
                }
            }.start();

            // 다이얼로그 생성
            AlertDialog.Builder builder = new AlertDialog.Builder(this);
            builder.setTitle("도움요청 메시지")
                    .setMessage("반복 재생 중입니다. 취소하시겠습니까?")
                    .setCancelable(false)
